...
export async function getDashboardMetrics({ from, to } = {}) {
  ...
  if (filterFrom && filterTo) {
    ...
    // Default behavior:
    // Payments: All time (no filter) -> actually, let's keep it all time if no filter
    // Customers: This month
    customerQuery.createdAt = { $gte: startOfMonth, $lt: startOfNextMonth };
    // Attendance: Today
    attendanceQuery.checkInTime = { $gte: startOfToday, $lt: endOfToday };
  }

  const [totalCustomers, newCustomersCount, paymentsAgg, activeCustomers] = await Promise.all([
