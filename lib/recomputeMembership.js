...
import { addMonths, addDays } from '@/lib/membership';
...
export async function recomputeMembershipForCustomer(customerId) {
  ...
  for (const p of payments) {
    ...
    let newEnd;
    // Prefer new fields, fallback to legacy membershipMonths
    if (p.durationType === 'days') {
      newEnd = addDays(base, p.durationValue || 1);
    } else {
      // Default to months (legacy or explicit)
      const months = p.durationValue || p.membershipMonths || 1;
      newEnd = addMonths(base, months);
    }
