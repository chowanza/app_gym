...
export const CustomerCreateSchema = z.object({
  ...
  membershipType: z.string().optional(), // Permitir cualquier string para planes dinámicos
...
export const PaymentCreateSchema = z.object({
  ...
  membershipMonths: z.number().int().positive().optional(), // Deprecated
  durationValue: z.number().int().positive().default(1),
  durationType: z.enum(['days', 'months']).default('months'),
}).refine((v)=> v.paymentMethod !== 'Pago Movil' || (v.referenceNumber && String(v.referenceNumber).trim().length > 0), {
  message: 'referenceNumber: Requerido para Pago Movil',
});
